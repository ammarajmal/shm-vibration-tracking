...
def main():
    ...
    for _, r in tqdm(df_frames.iterrows(), total=len(df_frames), desc=f"Robust Detect {args.cam}"):
        t_sec = float(r["t_sec"])
        ...
        if img is None:
            # Add failed row
            rows.append({T: t_sec, CAM: args.cam, DETECTED: 0, SOURCE: "none"})
            prev_img = None
            prev_corners = None
            continue
        ...
        dets = detect_tags(img_gray, family=args.family, params=det_params) # detect_tags handles grayscale check usually but better explicit
        ...
        elif args.use_klt and prev_corners is not None and prev_img is not None:
        ...
        if source in ["tag", "klt"] and current_corners is not None:
            pose, reproj = solve_pnp_from_corners(current_corners, caminfo, args.tag_size_m, use_ransac=False)
        ...
        flow_mag = np.nan
        if args.use_flow and prev_img is not None:
        ...
        if source in ["tag", "klt"]:
            ...
            # Or keep trying to track from last known?
            # Usually KLT fails if steps are too large.
            # If we lost it, better to wait for re-detection.
            pass
        ...
        row = {
            T: t_sec,
            CAM: args.cam,
            TAG: int(args.tag_id) if args.tag_id else int(current_tag_id),
            **pose_data,
